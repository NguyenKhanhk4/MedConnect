...
import "./MultiSpecializationBooking.css";

const { Title, Text, Paragraph } = Typography;
const { Step } = Steps;
const { RangePicker } = DatePicker;

const MultiSpecializationBooking = () => {
    ...
    try {
      ...
      if (response?.success || response?.data?.success) {
        ...
        if (visit) {
          ...
          setCurrentStep(1);
          message.success("Đã chọn ngày khám. Vui lòng chọn chuyên khoa và bác sĩ.");
        } else {
  ...
  const handleCompletePlanning = async () => {
    ...
    try {
      ...
      if (response?.success || response?.data?.success) {
        const visit = response?.data?.visit || response?.visit;
        const savedAppointments = response?.data?.appointments || response?.appointments || [];
        ...
        if (response?.data?.conflicts?.length > 0 || response?.conflicts?.length > 0) {
          const conflictsList = response?.data?.conflicts || response?.conflicts || [];
  ...
  const appointmentColumns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        if (record.status === "pending_doctor" || record.status === "accepted") {
          return (
            <Button
              size="small"
              danger
  ...
  const handleCancelAppointment = async (appointmentId) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await api.post("/api/medical-visits/cancel-appointment", {
            appointmentId,
          });
          ...
          <Card>
            ...
            {selectedSpecializations.map((spec) => (
              ...
              >
                ...
                {(() => {
                  const specAppointments = appointments?.filter((apt) => {
                    ...
                    const match = aptSpecIds.some((sid) => {
                      ...
                      const matchResult = specId?.toString() === currentSpecId?.toString();
                      if (matchResult) {
                        console.log("Matched appointment:", apt._id, "with spec:", currentSpecId);
                      }
                      return matchResult;
                    });
                  ...
                  console.log(`Specialization ${spec.name} appointments:`, specAppointments);
                    ...
                    >
                        <Space direction="vertical" style={{ width: "100%" }}>
                          <Space>
                            ...
                            <Text>
                              {apt.scheduledStart 
                                ? `${dayjs(apt.scheduledStart).format("HH:mm")} - ${dayjs(apt.scheduledEnd).format("HH:mm")}`
                                : "N/A"}
                            </Text>
                          ...
                          {apt.mode && (
                            <Text type="secondary">
                              Hình thức: {apt.mode === "online" ? "Trực tuyến" : "Tại phòng khám"}
                            </Text>
                          )}
                        </Space>
                      </Card>
            ...
            <Space style={{ marginTop: 24, width: "100%" }} justify="space-between">
...
export default MultiSpecializationBooking;
